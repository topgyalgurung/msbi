-- adventureworks2014 --
-- runs very slow
SELECT cr.[Name] AS CountryRegionName 
,SUM(sd.LineTotal)AS SalesAmount 
FROM Sales.SalesOrderDetail sd  
INNER JOIN Sales.SalesOrderHeader sh ON sd.SalesOrderID = sd.SalesOrderID 
INNER JOIN Sales.Customer cu ON sh.CustomerID = cu.CustomerID 
INNER JOIN Person.Person pe ON cu.PersonID = cu.PersonID 
INNER JOIN Person.BusinessEntity be ON pe.BusinessEntityID = be.BusinessEntityID 
INNER JOIN Person.BusinessEntityAddress bea ON bea.BusinessEntityID = bea.BusinessEntityID 
INNER JOIN Person.[Address] ad ON ad.AddressID = bea.AddressID 
INNER JOIN Person.StateProvince sp ON sp.StateProvinceID = ad.StateProvinceID 
INNER JOIN Person.CountryRegion cr ON cr.CountryRegionCode = sp.CountryRegionCode 
GROUP BY cr.Name 
ORDER BY cr.Name 

-- adventureworksdw2014 runs fast
SELECT dg.EnglishCountryRegionName AS CountryRegionName 
,SUM(fis.SalesAmount)AS SalesAmount 
FROM FactInternetSales fis  
INNER JOIN DimCustomer dc ON fis.CustomerKey = dc.CustomerKey 
INNER JOIN DimGeography dg ON dc.GeographyKey = dc.GeographyKey 
...
SELECT T0.CustomerID,
	CONCAT(T1.FirstName,'',T1.LastName) AS CustomerName,
	T3.Name AS CountryRegion
FROM AdventureWorks2014.Sales.Customer T0
INNER JOIN AdventureWorks2014.Person.Person T1 ON T0.PersonID=T1.BusinessEntityID
INNER JOIN AdventureWorks2014.Sales.SalesTerritory T2 ON T0.TerritoryID=T2.TerritoryID
INNER JOIN AdventureWorks2014.Person.CountryRegion T3 ON T2.CountryRegionCode=T3.CountryRegionCode
